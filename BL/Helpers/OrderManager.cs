...
internal static class OrderManager
{
    ...
    internal static void Create(BO.Order boOrder)
    {
        ...
        try
        {
            ...
            // 2. Calculate Coordinates using Tools with caching
            double lat = 0, lon = 0;
            try
            {
                Tools.GetCoordinates(boOrder.CustomerAddress, out lat, out lon, 
        ...
        lock (AdminManager.BlMutex) //stage 7
        {
            ...
            return closedOrders.Select(item =>
            {
                ...
                if (actualDistance == null && item.Order != null)
                {
                    ...
                    // Try to get cached distance first (non-blocking)
                    if (Tools.TryGetCachedRouteDistance(
                        compLat, 
                        compLon, 
                        item.Order.Latitude, 
                        item.Order.Longitude, 
                        (BO.Vehicle)item.Delivery.VehicleType, 
                        out double cachedDistance))
                    {
                        actualDistance = cachedDistance;
                    }
                    else
                    {
                        // If not in cache, use aerial distance for historical data
